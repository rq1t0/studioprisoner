...
export async function generateMetadata({ params }: { params: { slug: string } }) {
  ...
  return {
    ...
    alternates: { canonical: `/works/${params.slug}` }
  };
}
...
export default async function WorkDetail({ params }: { params: { slug: string } }) {
  ...
  const breadcrumbLd = {
    ...
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'WORKS', item: '/works' },
      { '@type': 'ListItem', position: 2, name: `${work.artist} – ${work.title}`, item: `/works/${work.slug}` }
    ]
  };
  ...
  const creativeLd = {
    '@context': 'https://schema.org',
    '@type': 'MusicRecording',
    name: `${work.artist} – ${work.title}`,
    byArtist: { '@type': 'MusicGroup', name: work.artist },
    inLanguage: 'ja-JP',
      ...
      <Section className="pb-6">
        <nav className="mb-3 text-sm text-foreground/70" aria-label="Breadcrumb">
          <ol className="flex gap-2">
            <li><Link href="/works" className="underline underline-offset-4">WORKS</Link></li>
            <li>/</li>
                    ...
                    <li key={t.title + i} className="rounded-lg border border-border/60">
                      <details>
                        ...
                        <div className="space-y-3 px-3 pb-3 text-sm">
                          ...
                          {!t.youtubeId && t.url && (
                            <div>
                              <a href={t.url} target="_blank" rel="noreferrer" className="underline underline-offset-4">参考リンクを開く</a>
                            </div>
                          )}
                        </div>
                      </details>
      ...
      <Section className="pt-0">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex gap-4 text-sm">
            {prev ? (
              <Link href={`/works/${prev.slug}`} className="underline underline-offset-4">
                ← {prev.artist} – {prev.title}
              </Link>
            ) : (
              <span className="text-foreground/50">先頭</span>
            )}
            {next ? (
              <Link href={`/works/${next.slug}`} className="underline underline-offset-4">
                {next.artist} – {next.title} →
              </Link>
            ) : (
              <span className="text-foreground/50">最後</span>
            )}
          </div>
          <Link href="/works" className="underline underline-offset-4">一覧へ戻る</Link>
        </div>
      </Section>
